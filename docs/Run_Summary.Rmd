...
Regs_out <- df3 %>%
  ...
  tidyr::separate_wider_delim(SeasonO," - ", names=c("S1","S2")) %>%
   mutate(S1=lubridate::ymd(S1),
          S2=lubridate::ymd(S2)) %>%
   mutate(Season=paste0(format(S1,format="%B %d")," - ", format(S2,format="%B %d"))) %>%
